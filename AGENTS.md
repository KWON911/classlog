...
> 이 문서는 Classlog의 Codex 작업 기준 문서다. 현재 코드·마이그레이션·`docs/superpowers/specs/`에 구현된 실제 구조를 우선하며, `README.md`는 초기 MVP 설명이 남아 있을 수 있으므로 현재 범위의 기준 문서로 사용하지 않는다.

### Codex 작업 환경

- 저장소별 작업 지침은 이 `AGENTS.md`만 사용한다. Claude 전용 설정과 지침은 유지하지 않는다.
- 명령 실행 권한과 외부 접근은 Codex의 샌드박스·승인 흐름을 따른다. 저장소 안에 도구별 권한 허용 목록을 중복 관리하지 않는다.
- 개발 서버는 기존 `npm run dev` 명령으로 실행한다. 별도의 도구 전용 launch 설정은 만들지 않는다.
