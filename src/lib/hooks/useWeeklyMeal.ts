...
import { buildWeeklyMeal, getMealsForRange } from '../services/neis-service'
import { weekdaysOf } from '../utils/date-utils'
import type { SchoolSettings, WeeklyMealDay } from '../types'
import type { WeeklyFetchStatus } from './useWeeklyTimetable'

export function useWeeklyMeal(settings: SchoolSettings | null, weekStart: Date, refreshToken: number) {
  const [status, setStatus] = useState<WeeklyFetchStatus>('idle')
  const [days, setDays] = useState<WeeklyMealDay[]>([])
  ...
  useEffect(() => {
    ...
    getMealsForRange(settings, weekdays[0], weekdays[4], { force }).then((result) => {
      ...
      setDays(buildWeeklyMeal(weekdays, result.data))
      setStatus('success')
    })
